...
let of_character_list cl =
  let r = ref cl in
  String.init (List.length cl) (fun _ ->
    let c = List.hd !r in  
    r := List.tl !r;
    c)
...
let set s ~index ~v =
  if index > length s - 1
  then None
  else Some (String.mapi (fun i c -> if i = index then v else c) s)
...
let concat ?(sep="") sl = StringLabels.concat ~sep sl
...
let iter t ~f = StringLabels.iter t ~f
let iteri t ~f = StringLabels.iteri t ~f
...
let rev t =
  ...
  | lgth ->
      let o = lgth - 1 in
      String.mapi (fun i _ -> String.get t (o - i)) t

let map t ~f = StringLabels.map t ~f
...
let map2_exn t1 t2 ~f =
  ...
  | lgth1, lgth2 ->
      String.mapi (fun i c -> f c (String.get t2 i)) t1

let mapi t ~f = StringLabels.mapi t ~f
